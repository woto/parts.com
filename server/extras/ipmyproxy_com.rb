ActiveRecord::Base.logger = ActiveSupport::BufferedLogger.new(File.join(Rails.root, 'log', 'daemon', 'ipmyproxy_com', "#{Rails.env}.log"))
Rails.logger = ActiveSupport::BufferedLogger.new(File.join(Rails.root, 'log', 'daemon', 'ipmyproxy_com', "#{Rails.env}.log"))

class IpmyproxyCom
  def perform
    while true
      Proxy.where('good > ?', -9).each do |proxy|
        begin
